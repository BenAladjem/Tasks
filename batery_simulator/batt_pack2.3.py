...
class WaterMeter():
    ...
    def __init__(self, bat_cap, src_time, src_cap, slp_time, wrk_time, type_rep, bsd):
        ...
        self.iter_time = Calculations.one_iteration_time(self.type_rep, self.AVG_CURRENT_TIME_REPORT, self.slp_time, self.wrk_time, src_time)
...
class FB():
    ...
    def __init__(self, bat_cap, src_time, src_cap, slp_time, wrk_time, type_rep, bsd):
        ...
        self.iter_time = Calculations.one_iteration_time(self.type_rep, self.AVG_CURRENT_TIME_REPORT, self.slp_time, self.wrk_time,
                                                         src_time)
        self.iter_cap = Calculations.one_iteration_capacity(self.type_rep, self.rep_energy, self.slp_energy,
                                                            self.wrk_energy, self.src_energy)
        self.batt_life = Calculations.calculate_time(self.bat_cap, self.iter_cap, self.iter_time, self.bsd)
...
class FC():
    ...
    AVG_CURRENT_CONSUMPTION_SLEEP = 1.6  # mA
...
class TestDevice():
    ...
    def __init__(self, bat_cap, src_time, src_cap, slp_time, slp_cap, wrk_time, wrk_cap, type_rep, bsd):
        ...
        self.src_cap = src_cap  # въвежда се от менюто
        self.type_rep = type_rep  # въвежда се от менюто
        self.wrk_time = wrk_time  # време за работа(измерване)
        self.src_time = src_time  # време за репорт
        self.slp_time = slp_time  # време за сън
        ...
        self.iter_time = Calculations.one_iteration_time(self.type_rep, self.rep_time, self.slp_time, self.wrk_time, self.src_time)
        self.iter_cap = Calculations.one_iteration_capacity(self.type_rep, self.rep_energy, self.slp_energy,
                                                            self.wrk_energy, self.src_energy)
        self.batt_life = Calculations.calculate_time(self.bat_cap, self.iter_cap, self.iter_time, self.bsd)
        self.batt_life_time = Calculations.time_convert(
...
def print_result():
    ...
    if class_device == TestDevice:
        ...
        dev = class_device(batt_capacity, search_time/3600,search_cap, sleep_time, work_time, type_report, batt_self_discharge)

...
def generate_report():
    ...
    print(f"Number protocol: {num}")


    num_protocol = num
    data = date_format
    device = combo.get()
    batt_capacity = txt_batt_cap.get()
    report_time = txt_rep_time.get()
    report_cap = 0
    work_time = txt_wrk_time.get()
    work_cap = 0
    sleep_time = txt_slp_time.get()
    sleep_cap = 0
    batt_self_discharge = txt_bsd.get()
    cycling_mode = info[1]
    batt_life_time = 0
    iterations = 0
